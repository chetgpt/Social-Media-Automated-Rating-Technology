...
def load_recorded_elements(folder_path=None):
    """Loads the recorded elements from the JSON file."""
    try:
        filename = "recorded_elements.json"
        if folder_path:
            filename = os.path.join(folder_path, filename)

        if not os.path.exists(filename):
            # Try the old filename as fallback
            old_filename = os.path.join(folder_path, "recorded_element.json") if folder_path else "recorded_element.json"
            if os.path.exists(old_filename):
                filename = old_filename
            else:
                print(f"[ERROR] Recorded elements file not found at {filename}")
                return None

        with open(filename, "r", encoding="utf-8") as f:
            elements = json.load(f)

        print(f"[INFO] Successfully loaded recorded elements from {filename}")
        return elements
    except Exception as e:
        print(f"[ERROR] Failed to load recorded elements: {e}")
        return None

async def extract_total_comment_count(page, recorded_elements):
    """
    Extracts the total comment count from the page using recorded elements.
    Tries multiple approaches to get the count.
    """
    try:
        count = 0

        # Check if recorded_elements is valid
        if not recorded_elements:
            print("[INFO] No recorded elements available, using JavaScript fallback")
            try:
                count = await page.evaluate(total_comment_count_js)
                print(f"[INFO] Got total comment count from JavaScript: {count}")
                return count or 0
            except Exception as e:
                print(f"[WARNING] JavaScript fallback failed: {e}")
                return 0

        # First try the comment_count element (usually shows "Comments (123)")
        if "comment_count" in recorded_elements:
            selector = recorded_elements["comment_count"]["css_selector"]
            try:
                # Try to get the element and extract its text
                element = page.locator(selector)
                if await element.count() > 0:
                    text = await element.inner_text()
                    print(f"[INFO] Found comment count element with text: {text}")

                    # Extract the number from text like "Comments (123)"
                    match = re.search(r"\((\d+[.,]?\d*[KkMmBb]?)\)", text)
                    if match:
                        count_text = match.group(1)
                        print(f"[INFO] Extracted count: {count_text}")

                        # Handle K, M, B suffixes
                        if any(suffix in count_text for suffix in ['K', 'k']):
                            count = int(float(re.sub('[Kk]', '', count_text)) * 1000)
                        elif any(suffix in count_text for suffix in ['M', 'm']):
                            count = int(float(re.sub('[Mm]', '', count_text)) * 1000000)
                        elif any(suffix in count_text for suffix in ['B', 'b']):
                            count = int(float(re.sub('[Bb]', '', count_text)) * 1000000000)
                        else:
                            count = int(re.sub(r'[,.]', '', count_text))

                        print(f"[INFO] Parsed total comment count: {count}")
                        return count
            except Exception as e:
                print(f"[WARNING] Error getting count from comment_count element: {e}")

        # If that fails, try the comment_button element (usually just shows the number)
        if "comment_button" in recorded_elements and count == 0:
            selector = recorded_elements["comment_button"]["css_selector"]
            try:
                # Try to get the element and extract its text
                element = page.locator(selector)
                if await element.count() > 0:
                    text = await element.inner_text()
                    print(f"[INFO] Found comment button with text: {text}")

                    # The text might just be the number
                ...
                print(f"[WARNING] Error getting count from comment_button element: {e}")

        # If both methods fail, try using the fallback JavaScript method
        if count == 0:
            print("[INFO] Both recorded elements failed to provide a comment count, falling back to JavaScript method")
            try:
                count = await page.evaluate(total_comment_count_js)
                print(f"[INFO] Got total comment count from JavaScript method: {count}")
                return count or 0
            except Exception as e:
                print(f"[WARNING] JavaScript fallback also failed: {e}")
                return 0

        return count
    except Exception as e:
        print(f"[ERROR] Failed to extract total comment count: {e}")
        return 0
...
(async () => {
    ...
    async function extractCommentsNewMethod() {
        ...
        // Find all potential comment items across the page
        const commentContainers = document.querySelectorAll('div[class*="DivCommentItemContainer"], div[class*="CommentItem"]');

        console.log(`Found ${commentContainers.length} potential comment items`);

        for (const item of commentContainers) {
            try {
                // Skip if already processed
                if (item.hasAttribute('data-processed') && item.getAttribute('data-processed') === 'true') {
                    continue;
                }

                console.log(`Processing comment item: ${item.className}, children: ${item.children.length}`);

                // Try to find username - multiple possible selectors
                let usernameEl = item.querySelector('[data-e2e*="username"], [data-e2e*="nickname"], a[href^="/@"]');

                if (!usernameEl) {
                    // Try a more generic approach - look for any link that might be a username
                    const links = Array.from(item.querySelectorAll('a')).filter(a =>
                        a.href && a.href.includes('/@') ||
                        a.getAttribute('href') && a.getAttribute('href').includes('/@')
                    );
                ...
                // First try to find a div that contains the text but excludes username
                const textContainers = Array.from(item.querySelectorAll('div')).filter(div => {
                    const text = div.textContent.trim();
                    return text.length > 10 && !text.startsWith(username);
                });

                if (textContainers.length > 0) {
                    // Sort by text length to find the most likely comment text
                    const container = textContainers.sort((a, b) => b.textContent.length - a.textContent.length)[0];
                    commentText = container.textContent.trim();
                } else {
                    // If no good div found, use the item's text minus the username
                    commentText = item.textContent.replace(username, '').trim();
                }

                // Try to find timestamp
                let timeStamp = null;
                const timeContainers = Array.from(item.querySelectorAll('span')).filter(span =>
                    /\d+[smhdwy] ago/.test(span.textContent) || span.textContent.includes('ago')
                );

                if (timeContainers.length > 0) {
                    timeStamp = timeContainers[0].textContent.trim();
                }
                ...
                console.log(`Error processing comment: ${error.message}`);
            }
        }
    ...
    // Use the target comment count passed from Python
    const targetCommentCount = PYTHON_TARGET_COMMENT_COUNT;
    ...
    if (container) {
        ...
        for (let attempt = 0; attempt < 20; attempt++) {
            ...
            if (targetCommentCount > 0 && currentCount >= targetCommentCount) {
                console.log(`Reached target of ${targetCommentCount} comments. Stopping.`);
                break;
            }
...
# Create a separate JavaScript function for detecting comment count
total_comment_count_js = r"""
...
async def main(record_mode=False, target_percentage=10):
    ...
    async with async_playwright() as p:
        ...
        try:
            await page.wait_for_load_state("networkidle", timeout=30000)  # Increase timeout
        except Exception as e:
            print(f"[WARNING] Timeout waiting for network idle: {e}")
            print("[INFO] Continuing despite network timeout...")
            await page.wait_for_timeout(5000)  # Wait a fixed time instead
        ...
        # Load recorded elements for comment count detection
        recorded_elements = load_recorded_elements(logs_folder)
        ...
        for i in range(num_videos):
            ...
            print(f"[INFO] Detected total comment count for video {i+1}: {total_comment_count}")

            # Calculate target (10% of total)
            target_count = max(1, math.ceil(total_comment_count * target_percentage / 100)) if total_comment_count > 0 else 100
            print(f"[INFO] Target comment count ({target_percentage}%): {target_count}")

            # Execute the scraping script to extract (and reveal more) comments
            try:
                print("\n[INFO] Executing comment scraping script...")

                # Reset log capture for this video
                logs = []
                page.on("console", lambda msg: logs.append(f"[BROWSER] {msg.type}: {msg.text}"))

                # Now execute the main scraping script with the target count
                # Replace the placeholder in the script with the actual target count
                script_with_target = scrape_js.replace("const targetCommentCount = PYTHON_TARGET_COMMENT_COUNT;",
                                                      f"const targetCommentCount = {target_count};")

                # Execute the scraping script with the target count
                comments = await page.evaluate(script_with_target)
                ...
                if not comments:
                    print("[ERROR] No comments were returned from the scraping script.")
                    continue

                # Calculate the percentage of comments we scraped
                if total_comment_count > 0:
                    percentage = round((len(comments) / total_comment_count) * 100, 2)
                else:
                    percentage = 0
                ...
                if isinstance(comments, list):
                    ...
                    if total_comment_count > 0:
                        percentage = round((len(comments) / total_comment_count) * 100, 2)
                        print(f"[INFO] Scraped {percentage}% of total comments ({len(comments)} of {total_comment_count})")
                    else:
                        percentage = 0
                        print("[INFO] Unknown percentage - couldn't detect total comment count")
