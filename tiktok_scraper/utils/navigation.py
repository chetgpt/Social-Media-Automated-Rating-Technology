...
async def click_next_video(page, selector):
    ...
    if not buttons:
        ...
        if not buttons:
            print("[INFO] No navigation buttons found. Using ArrowDown native shortcut to move to next video...")
            try:
                # Use TikTok's native ArrowDown shortcut which is much more reliable
                await page.keyboard.press("ArrowDown")
                await page.wait_for_timeout(3000)  # Wait for potential load
                print("[INFO] Pressed ArrowDown to reveal next video")
                return True  # Assume scrolling may have worked
            except Exception as e:
                print(f"[ERROR] Failed to press ArrowDown: {e}")
                return False
