...
def _post_page(
    ...
    *,
    ...
    if next_cursor is None:
        next_cursor = (
            str(int(request_cursor) + max(1, len(rows))) if has_more else request_cursor
        )
    query = {
        "sec_uid": sec_uid,
...
def _install_capture(
    monkeypatch, integration, *, pages, identities=None, stop_reason="fixture"
):
...
def test_finite_profile_inventory_dedupes_pinned_and_rejects_other_owners(monkeypatch):
    ...
    assert records[0]["url"] == ("https://www.tiktok.com/@creator.name/video/100")
    ...
    assert records[1]["url"] == ("https://www.tiktok.com/@creator.name/photo/101")
...
def test_all_profile_inventory_requires_and_records_terminal_has_more_false(
    monkeypatch,
):
    integration = TikTokAPIIntegration(enable_api=False)
    pages = [
...
def test_owner_can_be_verified_by_bound_stable_id_when_item_omits_handle(monkeypatch):
    ...
    assert (
        integration.last_creator_profile_diagnostics["rejected_owner_unverified"] == 0
    )


def test_conflicting_stable_creator_id_is_rejected_instead_of_widening_owner(
    monkeypatch,
):
    integration = TikTokAPIIntegration(enable_api=False)
    _install_capture(
    ...
    assert diagnostics["stop_reason"] == ("all_profile_rows_rejected_by_owner_fence")
...
def test_missing_request_identity_binding_is_not_a_verified_empty_profile(monkeypatch):
    ...
    page["url"] = "https://www.tiktok.com/api/post/item_list/?cursor=0&count=35"
    _install_capture(
        monkeypatch,
        integration,
        pages=[page],
...
def test_direct_html_refresh_preserves_or_detects_photo_media_type(
    ...
    if fresh_item_has_photo_structure:
        item["imagePost"]["images"].append({"imageURL": {"urlList": []}})
    payload = {
    ...
    class FakeRequest:
        async def get(self, url, **_kwargs):
            assert url == ("https://www.tiktok.com/@Creator.Name/photo/700")
            return FakeResponse()
    ...
    assert candidate["url"] == ("https://www.tiktok.com/@Creator.Name/photo/700")
...
def test_profile_capture_uses_page_scroll_and_stops_on_real_terminal_response():
    ...
    assert captured["post_pages"][0]["url"] == (
        "https://www.tiktok.com/api/post/item_list/" "?secUid=sec-creator-1&cursor=0"
    )
...
def test_profile_capture_page_cap_remains_nonterminal():
    integration = TikTokAPIIntegration(enable_api=False)
    page = _FakePage()
    ...
    diagnostics = captured["capture_diagnostics"]
    assert len(captured["post_pages"]) == 1
    assert diagnostics["latest_has_more"] is True
    assert diagnostics["terminal_response_observed"] is False
    assert diagnostics["stop_reason"] == "page_cap_reached"
