...
def creator_evidence(post_id, *, handle="maker", content_type="video"):
    record = {
        "id": str(post_id),
        "url": (f"https://www.tiktok.com/@{handle}/{content_type}/{post_id}"),
        "username": handle,
...
def test_creator_all_cli_rejects_page_cap_before_creating_database(
    tmp_path,
    capsys,
):
    database = tmp_path / "must-not-exist.sqlite"

    result = engage_tiktok.main(
        [
            "--database",
            str(database),
            "music-audit",
        ...
        with pytest.raises(CollectionIncompleteError, match="1/2"):
            asyncio.run(
                collect_exact(
                    ...
                    run_id=run_id,
                    preflight=ReadyPreflight(),
                    collector=CreatorInventoryCollector([creator_evidence("100")]),
                )
            )
        status = run_status(conn, run_id)
        assert status["status"] == "collection_incomplete"
        ...
        class WrongOwnerCollector:
            ...
            async def collect(
                ...
                *,
                ...
                self.last_diagnostics = {
                    "collection_stop_reason": ("creator_profile_inventory_exhausted")
                }
                return [changed]
...
def test_creator_inventory_state_rejects_terminal_snapshot_corruption(tmp_path):
    conn = connect_database(tmp_path / "creator.sqlite")
    try:
        run_id = create_run(
            conn,
            ...
            creator_handle="maker",
            collect_all=True,
            run_id="creator-corruption-gates",
        )
        asyncio.run(
            collect_exact(
                conn,
        ...
        original = dict(engage_tiktok._run_row(conn, run_id))

        corruptions = (
            (
                "profile_inventory_terminal=0",
                (),
                "inventory is not terminal",
            ),
            (
                "profile_inventory_count=profile_inventory_count+1",
                (),
        ...
        for assignment, parameters, message in corruptions:
            conn.execute(
                f"UPDATE engage_tiktok_runs SET {assignment} WHERE run_id=?",
                (*parameters, run_id),
            )
            conn.commit()
            with pytest.raises(StageGateError, match=message):
                engage_tiktok._assert_creator_inventory_state(
                    engage_tiktok._run_row(conn, run_id)
                )
            conn.execute(
                """
                UPDATE engage_tiktok_runs
                SET profile_inventory_terminal=?, profile_inventory_count=?,
                    requested_count=?, profile_inventory_hash=?
                WHERE run_id=?
                """,
                (
                    original["profile_inventory_terminal"],
                    original["profile_inventory_count"],
                    original["requested_count"],
                    original["profile_inventory_hash"],
                    run_id,
                ),
            )
            conn.commit()
...
def test_creator_all_nonterminal_callback_never_derives_requested_count(tmp_path):
    conn = connect_database(tmp_path / "creator.sqlite")
    try:
        run_id = create_run(
            conn,
            ...
            creator_handle="maker",
            collect_all=True,
            run_id="creator-nonterminal-count",
        )
        with pytest.raises(
            CollectionIncompleteError,
            match="creator_profile_frontier_not_terminal",
        ):
            asyncio.run(
                collect_exact(
                    conn,
...
def test_fixed_creator_nonterminal_inventory_keeps_fixed_count_label(tmp_path):
    conn = connect_database(tmp_path / "creator.sqlite")
    try:
        run_id = create_run(
            conn,
            ...
            source_mode="creator",
            creator_handle="maker",
            run_id="creator-fixed-nonterminal",
        )
        with pytest.raises(
            CollectionIncompleteError,
            match=r"0/5 .*creator inventory unresolved",
        ):
            asyncio.run(
                collect_exact(
                    conn,
...
def test_creator_inventory_rejects_globally_known_selected_ids(
    tmp_path,
    monkeypatch,
):
    master_path = tmp_path / "master.sqlite"
    ...
    monkeypatch.setattr(engage_tiktok, "known_post_ids", lambda *_: {"100"})
    try:
        run_id = create_run(
            conn,
            project="creator_test",
            ...
            creator_handle="maker",
            master_database=str(master_path.resolve()),
            run_id="creator-global-known-gate",
        )
        with pytest.raises(StageGateError, match="globally known IDs"):
            asyncio.run(
                collect_exact(
                    conn,
                    run_id=run_id,
...
def test_compile_csv_is_not_a_cli_command():
