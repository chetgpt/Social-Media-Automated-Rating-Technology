...
Copy-Item config\campaign_specs\template.json "config\campaign_specs\$Campaign.json"
```
...
```powershell
$Spec = "config\campaign_specs\client_campaign.json"
...
```powershell
python incremental_project.py `
  --campaign-spec config\campaign_specs\client_campaign.json `
  --social-browser
```
