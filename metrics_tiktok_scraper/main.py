...
def _load_tiktok_curl_cookies(capture_path):
    """Parse cookie values from a DevTools Copy-as-cURL (cmd) capture."""
    try:
        with open(capture_path, "r", encoding="utf-8", errors="replace") as handle:
            raw = handle.read()
    except OSError as exc:
        raise RuntimeError(f"Could not read TikTok cURL capture: {capture_path}") from exc

    match = re.search(r'(?m)^\s*-b\s+\^"(.+?)\^"\s*\^?\s*$', raw)
    if not match:
        raise RuntimeError("TikTok cURL capture does not contain a -b cookie header")

    cookie_header = match.group(1).replace("^", "").replace('\\"', '"')
    values = {}
...
async def process_videos_by_clicking(
    ...
    if api_integration and use_api and (video_elements or is_search_target or is_candidate_target):
        ...
        try:
            ...
            print(f"[WARN] TikTok date-window gate failed; continuing without filtering: {e}", flush=True)
        ...
        try:
            ...
            print(f"[WARN] TikTok relevance gate failed; continuing without filtering: {e}", flush=True)
        ...
        try:
            ...
            print(f"[WARN] TikTok known-content gate failed; continuing without filtering: {e}", flush=True)
...
async def main(
    ...
    # Avoid closing the user's normal Edge sessions unless explicitly requested.
    if os.environ.get("TIKTOK_CLOSE_EDGE", "0").strip().lower() in {"1", "true", "yes", "on"}:
        close_edge_tasks()
        time.sleep(2)
    else:
        print("[INFO] Leaving existing Edge windows untouched (set TIKTOK_CLOSE_EDGE=1 to force-close).")

    # Legacy CDP profile path. New callers should pass an explicit persistent
    # user-data directory and profile directory instead.
    configured_edge_profile = os.environ.get("TIKTOK_EDGE_PROFILE_PATH", "").strip()
    profile_path = configured_edge_profile or r"C:\Users\DELL\AppData\Local\Microsoft\Edge\User Data\Profile 2"
    user_data_dir = os.path.dirname(profile_path)
    legacy_profile_directory = os.path.basename(profile_path)
    persistent_user_data_dir = os.environ.get("TIKTOK_PLAYWRIGHT_USER_DATA_DIR", "").strip()
    requested_profile_directory = os.environ.get("TIKTOK_PROFILE_DIRECTORY", "").strip()
    configured_cdp_url = os.environ.get("TIKTOK_CDP_URL", "").strip()
    cookie_curl_file = os.environ.get("TIKTOK_COOKIE_CURL_FILE", "").strip()
    browser_channel = os.environ.get("TIKTOK_BROWSER_CHANNEL", "msedge").strip() or "msedge"
    strict_profile = _env_flag(
        "TIKTOK_PROFILE_STRICT",
        bool(
            configured_edge_profile
            or persistent_user_data_dir
            or requested_profile_directory
            or configured_cdp_url
        ),
    )
    require_auth = _env_flag("TIKTOK_REQUIRE_AUTH", False)
    ...
    cmd = [
        msedge_path,
        f"--profile-directory={legacy_profile_directory}",
    ...
    proc = None
    external_cdp = False
    use_cdp = bool(configured_cdp_url) or not _env_flag("TIKTOK_FORCE_PLAYWRIGHT", False)
    if use_cdp and not configured_cdp_url:
        ...
        if not wait_for_port("127.0.0.1", 9222, timeout=90):
            terminate_proc()
            proc = None
            message = "Remote debugging port did not open for the requested TikTok Edge profile"
            if strict_profile:
                raise RuntimeError(message + "; refusing to silently use a different profile")
            print(f"[WARN] {message}; falling back to the default Playwright profile.")
            use_cdp = False
        else:
    ...
    async with async_playwright() as p:
        if use_cdp:
            cdp_url = configured_cdp_url or "http://127.0.0.1:9222"
            try:
                browser = await p.chromium.connect_over_cdp(cdp_url)
                external_cdp = bool(configured_cdp_url)
            except Exception as e:
                terminate_proc()
                proc = None
                external_cdp = False
                if strict_profile:
                    raise RuntimeError(
                        f"Could not connect to requested TikTok CDP session at {cdp_url}; "
                        "refusing to silently use a different profile"
                    ) from e
                print("[WARN] Error connecting over CDP; falling back to Playwright Edge profile:", e)
                use_cdp = False
        ...
        if use_cdp:
            ...
            fallback_profile = persistent_user_data_dir or os.path.join(
                os.getcwd(), "comments_data", ".edge_tiktok_profile"
            )
            os.makedirs(fallback_profile, exist_ok=True)
            launch_args = ["--disable-blink-features=AutomationControlled"]
            if requested_profile_directory:
                launch_args.append(f"--profile-directory={requested_profile_directory}")
            profile_label = requested_profile_directory or "Default"
            print(
                f"[INFO] Launching Playwright {browser_channel} for TikTok: "
                f"user_data_dir={fallback_profile} profile={profile_label} strict={strict_profile}"
            )
            try:
                ...
                raise RuntimeError(
                    f"Could not launch requested TikTok profile '{profile_label}' from "
                    f"'{fallback_profile}'"
                ) from e
            browser = context
            pages = context.pages
            page = pages[0] if pages else await context.new_page()
            if cookie_curl_file:
                injected_cookies = _load_tiktok_curl_cookies(cookie_curl_file)
                await context.add_cookies(injected_cookies)
                print(
                    f"[INFO] Injected {len(injected_cookies)} TikTok cookies from cURL capture "
                    "(values redacted)."
                )
            await page.goto(initial_url, wait_until="domcontentloaded", timeout=45000)
        ...
        if target_url and re.search(r'/(video|photo)/\d+', target_url):
            ...
            if not comments:
                print("[ERROR] Direct API returned no comments for this video.")
                await browser.close()
                terminate_proc()
                return
            ...
            print(f"[SUCCESS] Direct API saved {len(comments)} comments for video ID {current_video_id}")
            await browser.close()
            terminate_proc()
            return
