...
COMBINED_COMMENT_FILE_BY_PLATFORM = {
    "youtube": "youtube_metrics.json",
    "instagram": "instagram_metrics.json",
    "facebook": "facebook_metrics.json",
    "x": "x_metrics.json",
}
...
def content_published_at(video: dict[str, Any], reference_time: Any = None) -> str:
    from metrics_tiktok_scraper.date_filter import parse_datetime
...
def content_key(platform: str, video: dict[str, Any], fallback: str = "") -> str:
    from metrics_tiktok_scraper.known_content import candidate_content_keys
...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    from metrics_tiktok_scraper.storage.google_drive import ensure_archive_schema
    from metrics_tiktok_scraper.profile_enrichment import init_profile_schema
...
def write_campaign_state(spec: dict[str, Any], paths: dict[str, Path]) -> tuple[Path, Path]:
    ...
    from metrics_tiktok_scraper.relevance import default_anchor_terms, extract_topic_tokens
...
def ingest_videos(
    ...
    *,
    ...
    from metrics_tiktok_scraper.profile_enrichment import (
    ...
    for index, raw_video in enumerate(videos, start=1):
        ...
        conn.execute(
            ...
            INSERT INTO content_items (
                ...
                transcript_attempt_count, first_seen_at, last_scraped_at,
...
def load_session_videos(session_path: Path, platform: str) -> list[dict[str, Any]]:
    ...
    if platform == "youtube":
        combined = comments_dir / "youtube_metrics.json"
        if combined.exists():
            return as_list(load_json(combined).get("videos"))
    if platform == "instagram":
        combined = comments_dir / "instagram_metrics.json"
        if combined.exists():
            return as_list(load_json(combined).get("videos"))
    if platform == "facebook":
        combined = comments_dir / "facebook_metrics.json"
        if combined.exists():
            return as_list(load_json(combined).get("videos"))
    if platform == "x":
        combined = comments_dir / "x_metrics.json"
        if combined.exists():
            return as_list(load_json(combined).get("videos"))
...
def project_date_windows(conn: sqlite3.Connection, project: str) -> list[dict[str, Any]]:
    from metrics_tiktok_scraper.date_filter import normalize_windows
...
def filter_content_rows_by_date(
    ...
    from metrics_tiktok_scraper.date_filter import filter_candidates_by_date
...
def compile_outputs(conn: sqlite3.Connection, project: str, keyword: str, paths: dict[str, Path], hours: int | None = None) -> dict[str, Any]:
    from metrics_tiktok_scraper.profile_contract import extract_profile_candidate
    from metrics_tiktok_scraper.profile_enrichment import (
        export_profiles,
        load_profile_lookup,
        profile_coverage,
    ...
    for platform in platforms:
        ...
        for index, content in enumerate(content_rows, start=1):
            ...
            from metrics_tiktok_scraper.raw_contract import finalize_content_record
    ...
    combined_path = output_dir / "combined_all_platforms_metrics.json"
    flat_path = output_dir / "combined_all_platforms_metrics_flat.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--social-browser",
        action="store_true",
        help="Use the running dedicated social browser recorded by social_browser.py.",
    )
    parser.add_argument(
...
def main() -> None:
    args = parse_args()
    if args.social_browser and not args.social_browser_cdp_url:
        social_state_path = Path(args.social_browser_state)
        try:
            social_state = json.loads(social_state_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise SystemExit(
                f"Dedicated social browser state is unavailable at {social_state_path}. "
                "Run: python social_browser.py start"
            ) from exc
        args.social_browser_cdp_url = str(social_state.get("cdp_url") or "").strip()
        if not args.social_browser_cdp_url:
            raise SystemExit(f"Dedicated social browser state has no cdp_url: {social_state_path}")

    shared_cdp_url = args.social_browser_cdp_url.strip()
    if shared_cdp_url:
        if not args.tiktok_user_data_dir and not args.tiktok_cdp_url:
            args.tiktok_cdp_url = shared_cdp_url
        if not args.instagram_user_data_dir and not args.instagram_cdp_url:
            args.instagram_cdp_url = shared_cdp_url
        if not args.facebook_user_data_dir and not args.facebook_cdp_url:
            args.facebook_cdp_url = shared_cdp_url
        if not args.x_user_data_dir and not args.x_cdp_url:
            args.x_cdp_url = shared_cdp_url

    campaign = None
    campaign_spec_path = None
    campaign_profile_path = None
    if args.campaign_spec:
        from campaign_spec import campaign_keywords, campaign_spec_digest, expand_campaign_sources, load_campaign_spec
    ...
    from metrics_tiktok_scraper.storage.google_drive import (
        DriveArchiveConfig,
        DriveArchiveError,
    ...
    from metrics_tiktok_scraper.profile_enrichment import backfill_profile_queue

    profile_queue_backfill = backfill_profile_queue(conn, args.project)

    run_results = []
    if not args.compile_only:
        platforms = ["youtube", "tiktok", "instagram", "facebook", "x"] if args.platform == "all" else [args.platform]
        if campaign:
    ...
    if args.profile_enrichment:
        ...
        from metrics_tiktok_scraper.profile_enrichment import enrich_queued_profiles
