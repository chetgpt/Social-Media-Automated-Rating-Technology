...
async def dummy_extract(v, **kwargs):
    res = dict(v)
    res.setdefault("comments", [])
    return res

import os
import re
from metrics_tiktok_scraper.main import main as run_scraper
...
def parse_arguments():
    ...
    parser.add_argument("--instagram-discovery-only", action="store_true",
                        help="Only discover Instagram posts and write instagram_metrics.json without scraping comments")
...
def main():
    ...
    if args.analyze:
        ...
        try:
            ...
            from metrics_tiktok_scraper.ai_analytics import run_analysis_pipeline
    ...
    from metrics_tiktok_scraper.raw_contract import finalize_content_record, finalize_payload
    ...
    if args.relevance_mode != "off" or args.relevance_profile:
        from metrics_tiktok_scraper.relevance import load_topic_profile
    ...
    from metrics_tiktok_scraper.date_filter import filter_candidates_by_date, windows_from_env, write_date_audit
    ...
    def apply_relevance_gate(candidates, platform, folders):
        if args.relevance_mode == "off" or not relevance_profile:
            return candidates
        from metrics_tiktok_scraper.relevance import filter_candidates, write_relevance_audit
    ...
    if args.known_content_file:
        from metrics_tiktok_scraper.known_content import load_known_content_file
    ...
    def apply_known_content_gate(candidates, platform, folders):
        if not known_content_keys:
            return candidates
        from metrics_tiktok_scraper.known_content import filter_new_candidates, write_known_content_audit
    ...
    async def run_yt(session_override=None):
        from metrics_tiktok_scraper.scrapers.youtube_scraper import YouTubeScraper
        from metrics_tiktok_scraper.utils.file_utils import create_session_folders
        from metrics_tiktok_scraper.main import save_comments_to_file
        ...
        async def fetch_youtube_video(i, v):
            async with semaphore:
                scrape_coro = dummy_extract(v, max_comments=args.comments if args.comments > 0 else 0)
                try:
        ...
        combined_json_path = os.path.join(folders["comments"], "youtube_metrics.json")
    ...
    async def run_ig(session_override=None):
        from metrics_tiktok_scraper.scrapers.instagram_scraper import InstagramScraper
        from metrics_tiktok_scraper.utils.file_utils import create_session_folders
        ...
        try:
            ...
            combined_json_path = os.path.join(folders["comments"], "instagram_metrics.json")
    ...
    async def run_x(session_override=None):
        from metrics_tiktok_scraper.main import save_comments_to_file
        from metrics_tiktok_scraper.scrapers.x_scraper import XScraper
        from metrics_tiktok_scraper.utils.file_utils import create_session_folders
        ...
        if extraction_mode == "official":
            ...
            from metrics_tiktok_scraper.scrapers.x_browser_scraper import XBrowserScraper
        ...
        try:
            ...
            for index, post in enumerate(posts, start=1):
                ...
                extraction = dummy_extract(post, **extraction_kwargs)
                try:
                    result = (
            ...
            combined_json_path = os.path.join(folders["comments"], "x_metrics.json")
    ...
    async def run_fb(session_override=None):
        from metrics_tiktok_scraper.scrapers.facebook_scraper import FacebookScraper
        from metrics_tiktok_scraper.utils.file_utils import create_session_folders
        from metrics_tiktok_scraper.main import save_comments_to_file
        ...
        try:
            ...
            combined_videos = [None] * len(videos)
            write_lock = asyncio.Lock()
            combined_json_path = os.path.join(folders["comments"], "facebook_metrics.json")
    ...
    async def orchestrate():
        ...
        if args.auto_analyze or args.platform == 'all':
            ...
            per_video_files = sorted(
                path
                for path in glob.glob(os.path.join(comments_path, "*_metrics.json"))
                if path not in aggregate_set and "combined_metrics.json" not in path
            )
            ...
            if combined_videos:
                combined_path = os.path.join(session_path, 'combined_metrics.json')
